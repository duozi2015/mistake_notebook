...
STUDENT_RULES = [
    AchievementRule("first_task", "首战告捷", "完成第一个任务", "🌟", "tasks_completed", 1, "个"),
    AchievementRule("task_10", "任务小能手", "累计完成 10 个任务", "🔥", "tasks_completed", 10, "个"),
    AchievementRule("task_50", "任务达人", "累计完成 50 个任务", "🏆", "tasks_completed", 50, "个"),
    AchievementRule("task_100", "任务大师", "累计完成 100 个任务", "👑", "tasks_completed", 100, "个"),
    AchievementRule("streak_3", "持之以恒·3天", "连续 3 天完成任务", "📅", "tasks_streak", 3, "天"),
    AchievementRule("streak_7", "持之以恒·7天", "连续 7 天完成任务", "📅", "tasks_streak", 7, "天"),
    AchievementRule("streak_30", "持之以恒·30天", "连续 30 天完成任务", "📅", "tasks_streak", 30, "天"),
    AchievementRule("stars_10", "星光闪耀·10星", "累计获得 10 星", "⭐", "stars_total", 10, "星"),
    AchievementRule("stars_50", "星光闪耀·50星", "累计获得 50 星", "⭐", "stars_total", 50, "星"),
    AchievementRule("self_task_5", "自主之星", "自主添加并完成 5 个任务", "🚀", "self_completed", 5, "个"),
    AchievementRule("mistake_10", "错题新手", "收录 10 道错题", "📚", "questions_total", 10, "道"),
    AchievementRule("mistake_50", "错题高手", "收录 50 道错题", "📚", "questions_total", 50, "道"),
    AchievementRule("review_7", "温故知新·7天", "连续 7 天复习错题", "🔁", "review_streak", 7, "天"),
    AchievementRule("review_30", "温故知新·30天", "连续 30 天复习错题", "🔁", "review_streak", 30, "天"),
]

PARENT_RULES = [
    AchievementRule("assign_first", "尽职尽责", "布置第一天任务", "👨‍👩‍👧", "assign_days", 1, "天"),
    AchievementRule("assign_7", "坚持布置·7天", "累计布置 7 天任务", "📋", "assign_days", 7, "天"),
    AchievementRule("assign_30", "坚持布置·30天", "累计布置 30 天任务", "📋", "assign_days", 30, "天"),
    AchievementRule("review_first", "首次批改", "完成第一次批改", "✍️", "reviews_count", 1, "次"),
    AchievementRule("review_10", "认真批改·10次", "累计批改 10 次", "✅", "reviews_count", 10, "次"),
    AchievementRule("review_50", "认真批改·50次", "累计批改 50 次", "✅", "reviews_count", 50, "次"),
    AchievementRule("timely_3", "及时反馈·3天", "连续 3 天当日批改", "⚡", "timely_streak", 3, "天"),
    AchievementRule("timely_7", "及时反馈·7天", "连续 7 天当日批改", "⚡", "timely_streak", 7, "天"),
    AchievementRule("stars_give_20", "送星大使", "累计送出 20 星", "⭐", "stars_given", 20, "星"),
]
...
def get_achievements(db: Session, user: User) -> list[dict]:
    ...
    for rule in rules:
        ...
        pct = round(val / rule.target * 100) if rule.target else 0
        result.append({
            "code": rule.code,
            "title": rule.title,
            "desc": rule.desc,
            "emoji": rule.emoji,
            "unlocked": val >= rule.target,
            "progress": val,
            "target": rule.target,
            "unit": rule.unit,
            "remaining": max(rule.target - val, 0),
